...
void CMainView::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	case Item::TEXT:
	{
		...
		if (t.DoModal() == IDOK) {
			...
			doc->m_ListenSocket.SendAllMessage((doc->list)[doc->list.size()-1]->DataToChar());
			Invalidate();
		}
		break;
	}
	case Item::IMAGE:
	{
		...
		if (i.DoModal() == IDOK) {
			...
			if (FAILED(hResult)) {
				...
				//int bufferSize = bmpDemension + sizeof(int) + sizeof(BYTE);
				//BYTE type = 0;
				//BYTE* bmpBuffer = new BYTE[bufferSize];
				//bitMap->GetBitmapBits(bmpDemension, bmpBuffer + sizeof(int) + sizeof(BYTE));
				//memcpy(bmpBuffer, &type, sizeof(BYTE)); // put the size into the head of package.
				//memcpy(bmpBuffer + 1, &bmpDemension, sizeof(int)); // put the size into the head of package.
...
void CMainView::DrawItems(CDC * p)
{
	CMFCApplication5Doc* doc = (CMFCApplication5Doc*)GetDocument();
